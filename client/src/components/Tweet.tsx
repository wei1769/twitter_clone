...
interface tweetState {
  ...
  setData: Dispatch<SetStateAction<tweetInterface[] | null>>;
}
const Tweet = (tweet: tweetState) => {
  const { currentUser } = useSelector((state: any) => state.user);

  const [userData, setUserData] = useState();

  const dateStr = formatDistance(new Date(tweet.tweet.timestamp), new Date());
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const findUser = await axios.get(`/users/find/${tweet.tweet.userId}`);
    ...
    fetchData();
  }, [tweet.tweet.userId, tweet.tweet.likes]);

  const handleLike = async (e: any) => {
    e.preventDefault();

    try {
      const like = await axios.put(`/tweets/${tweet.tweet.id}/like`, {
        id: currentUser._id,
      });
